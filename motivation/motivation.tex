...
The ratio is given in Equation \ref{eq:production}.
...
SHiP is a general-purpose search for dark sector particles \cite{04956}; E1067 is an upgrade to an existing dimuon spectrometer to allow parasitic searches for heavy photons \cite{00050}.
NA64 at CERN is an electron beam experiment that uses an electromagnetic calorimeter as an active dump \cite{6521}.
...
Two experiments have been run with existing fixed-target spectrometers: one experiment using the A1 spectrometer at MAMI \cite{5502}, and the APEX experiment using the HRS spectrometer at JLab Hall A (a test run was completed, a full run is scheduled) \cite{2557,2750}.
Two experiments are planned: DarkLight at the JLab LERF \cite{4717}, and MAGIX at MESA \cite{020006}.

A positron beam incident on a thin target will generate heavy photons through annihilation.
Experiments of this type can detect monophoton final states ($e^+e^- \to \gamma A' \to \gamma \mathrm{dark}$) and identify the heavy photon in the missing mass spectrum. Experiments are proposed at INFN Frascati (PADME \cite{01867}) and VEPP-3 \cite{5089}.

Proton or ion beams can also be used: the HADES spectrometer was used to measure $e^+e^-$ pairs from $p$-on-$p$, $p$-on-Nb, and Ar-on-KCl interactions.
The assumed signal was from meson decays \cite{0216}.

\subsection{Colliders}
Meson factories
annihilation, meson decay, dark higgsstrahlung

BaBar: $\upsilon(3S)$ dimuon \cite{081803} \cite{5263}
WASA: $\pi^0$ decay \cite{0671}
Belle-II: \cite{02089}
KLOE: $\phi$ factory, production through annihilation or meson decay, detection through lepton or pion pairs, vertexing possible \cite{3927,0411,06086,06795,00740,7772}
