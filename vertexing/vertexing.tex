...
\end{center}
\caption{Vertex $z$ for a resolution-limited mass slice (in data) centered at $m_{A'}=35.5$ MeV. A heavy photon signal would appear as a long rightwards tail (longer than the exponential fitted by the blue curve, which is the tail of the prompt vertex distribution).
...
Since the opening angle and momentum measurements predominantly rely on different parts of the tracker (opening angle on the upstream half of the tracker, momentum on the downstream half), their covariance is negligible.
...
\end{center}
    \caption{Mass resolution vs. $z$ for 40 MeV heavy photons.
...
This is the same plot as Figure \ref{fig:zvsmass}, but only plotting events above $z=z_{cut}$ (the black curve).
Some events are close to $z=z_{cut}$, but some events are substantially above the black line ($z>>z_{cut}$).}
...
\begin{figure}[ht]
\begin{center}
    \includegraphics[width=0.7\textwidth,page=60,angle=-90]{vertexing/figs/golden_mres}
\end{center}
\caption{The background CDF values (quantiles) for events in the mass slice centered at $m_{A'}=35.5$ MeV that have $z>z_{cut}$ (this is the same mass slice as Figure \ref{fig:vz_1d}).
...
The two populations of events noted in Figure \ref{fig:zvsmass_candidates} are also distinguishable here: events with $z$ close to $z_{cut}$ have quantiles spread across the range of 0 to 1, while events with $z>>z_{cut}$ have quantiles close to 1.
If only the events with quantiles less than 0.9 were counted, the mean number per mass slice of events with $z>z_{cut}$ would be 0.52.
}
...
A sample of trident Monte Carlo (with 1/7 as much integrated luminosity as the data used for this analysis) contains one similar event: see Figure \ref{fig:zvsmass_candidates_mc}.
This suggests that these events are a background that can be reproduced and understood in Monte Carlo, and it may be possible to identify a cut to reject them.
Since the current rate of these events is greater than the expected rate of heavy photon decays, this is essential to the full HPS analysis.
...
\end{center}
\caption{Predicted 90\% CL upper limit on the heavy photon production rate, projected for the full 2015 data set, using the full HPS acceptance, and assuming no events with $z>z_{cut}$. A value of 1 would mean exclusion; the lowest contour on this plot is 7, and the lowest point is 6.82.}
    \label{fig:upper_limit_fullset_alllayers}
\end{figure}

Some data was taken in 2015 with the tracker opened wider than its nominal position (silicon at 1.5 mm from the beam, instead of 0.5 mm).
...
The projected reach based on this analysis is substantially worse than the reach estimates in the 2014 proposal (see Figure \ref{fig:reach}), even if th the 1-week run that was 
